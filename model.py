...
class DualStreamTransformer(nn.Module):
    """
      Args:
        ...
        # Decoder 
        self.decoder = MultimodalDecoder(d_model, n_head, d_hid, num_encoder_layers, dropout)
    ...
    text_encoded = self.text_encoded(text_embedded, src_key_padding_mask=text_padding_mask)
    ...
    # Decoder pass
    decoder_output = self.decoder(tgt_embedded, text_encoded, image_encoded, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_padding_mask, text_memory_key_padding_mask=text_padding_mask, image_memory_key_padding_mask=image_padding_mask)
...
class BertTextEmbedding(nn.Module):
    def __init__(self, bert_model_name="bert-base-uncased"):
        super().__init__
        self.bert = BertModel.from_trained(bert_model_name)
...
class Encoder(nn.Module):
    def __init__(self, d_model: int, n_head: int, d_hid: int, n_layers: int, dropout: float = 0.1):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model, n_head, d_hid, dropout, activation = "gelu")
...
class DynamicGating(nn.Module):
    def __init__(self, d_model: int, dropout: float = 0.1):
        ...
        seld.dropout = nn.Dropout(dropout)
...
class MultimodalDecoderLayer(nn.Module):
    def __init__(self, d_model: int, n_head: int, d_hid: int, dropout: float = 0.1):
        ...
        self.cross_attn_text = nn.MultiheadAttention(d_model, n_head, dropout=dropout)
        # Cross Attention with Image
        self.cross_attn_image = nn.MultiheadAttention(d_model, n_head, dropout=dropout)
        ...
        text_attn, _ = self.cross_attn_text(tgt, text_memory, text_memory, key_padding_mask=text_memory_key_padding_mask)

        text_out = self.norm2(self.dropout(text_attn) + tgt)

        # Cross-attention to image
        if image_memory is not None:
            image_attn, _ = self.cross_attn_image(tgt, image_memory, image_memory, key_padding_mask=image_memory_key_padding_mask)
...
class MultimodalDecoder(nn.Module):
    ...
    def forward(self, tgt, text_memory, image_memory, tgt_mask,  tgt_key_padding_mask=None, text_memory_key_padding_mask=None, image_memory_key_padding_mask=None):
        ...
        for layer in self.layers:
            output = layer(output, text_memory, image_memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask, text_memory_key_padding_mask=text_memory_key_padding_mask, image_memory_key_padding_mask=image_memory_key_padding_mask)
