...
class Trainer:
    ...
    def _process_batch(self, batch):
        ...
        txt_dino   = torch.zeros(txt_ids.size(0), cap_dino.size(-1),
                                device=self.device, dtype=cap_dino.dtype)

        # print("txt ids ", self.tokenizer.decode(txt_ids[0]))
        # print("txt labels ", self.tokenizer.decode(txt_labels[0]))
        # print("txt attention ", txt_attn_mask[0])


        # print("cap ids ", self.tokenizer.decode(cap_ids[0]))
        # print("cap labels ", self.tokenizer.decode(cap_labels[0]))
        # print("cap attention ", cap_attn_mask[0])
        return txt_ids, txt_labels, txt_attn_mask, txt_dino, cap_ids, cap_labels, cap_attn_mask, cap_dino
    ...
    def train_step(self, batch):
        txt_ids, txt_labels, txt_attn_mask, _, cap_ids, cap_labels, cap_attn_mask, cap_dino = self._process_batch(batch)
