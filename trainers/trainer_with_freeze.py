...
class Trainer:
    ...
    def _freeze_multimodal(self):

        for p in self.model.image_embedding.parameters():
            p.requires_grad = False
        for p in self.model.image_encoder.parameters():
            p.requires_grad = False

        for layer in self.model.decoder.layers:
            for p in layer.cross_attn_txt_image.parameters():
                p.requires_grad = False
            for p in layer.gate.parameters():
                p.requires_grad = False

    def _unfreeze_multimodal(self):
        for p in self.model.image_embedding.parameters():
    ...
    def _freeze_text_only(self):
        for p in self.model.text_embedding.parameters():
            p.requires_grad = False

        for name, p in self.model.decoder.named_parameters():
            if ("cross_attn_txt_image" in name) or ("gate" in name):
                p.requires_grad = True
            else:
                p.requires_grad = False

    def _unfreeze_text_only(self):
        for p in self.model.text_embedding.parameters():
            p.requires_grad = True
    ...
    def train(self):
        ...
        if not self.unified:
            for epoch in range(self.start_epoch, self.max_epochs):
                if epoch % 2 == 0:
                    self._freeze_multimodal()
                    self._unfreeze_text_only()
                    loader = self.text_train_loader
                    train_use_image = False
                else:
                    self._unfreeze_multimodal()
                    self._freeze_text_only()
