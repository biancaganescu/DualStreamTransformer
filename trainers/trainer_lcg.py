...
class Trainer:
    ...
    def _setup_wandb(self, project, run_name, config):
        ...
        wandb.init(project=project, name=run_name, config=config, dir=self.checkpoint_dir)
    ...
    def _compute_loss(self, logits, targets, attention_mask=None, first_layer_output=None, image_features=None, use_image=False):
        ...
        if use_image and first_layer_output is not None and image_features is not None:
            ...
            lcg_loss = self.model.lcg_loss(first_layer_output, image_features, padding_mask)
    ...
    def train_step(self, batch, use_image):
        ...
        with autocast():
            ...
            loss, loss_ntp, lcg_loss = self._compute_loss(outputs, input_ids, attention_mask, first_layer_output, image_features, use_image)
        ...
        if not self.unified:
            for epoch in range(self.start_epoch, self.max_epochs):
                ...
                for batch in progress_bar:
                    loss, loss_ntp, lcg_loss = self.train_step(batch, use_image=train_use_image)
                    epoch_loss += loss
                ...
                epoch_lcg_loss = epoch_lcg_loss / max(lcg_loss_batch_count, 1) if train_use_image else None
    ...
    def evaluate_loader(self, loader, use_image, prefix):
        ...
        avg_lcg_loss = total_lcg_loss / max(lcg_loss_batch_count, 1) if use_image else None
