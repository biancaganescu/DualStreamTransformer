...
class Trainer:
    ...
    def _compute_loss(self, logits, targets, attention_mask=None, text_pooled=None, image_pooled=None, use_image=False):
        ...
        if use_image and text_pooled is not None and image_pooled is not None:
            ...
            contrastive_loss = self.model.compute_contrastive_loss(text_pooled, image_pooled, batch_size)
    ...
    def train_step(self, batch, use_image):
        ...
        with autocast():
            ...
            loss, loss_ntp, contrastive_loss = self._compute_loss(outputs, input_ids, attention_mask, text_pooled, image_pooled, use_image)
        ...
        return loss.item(), loss_ntp.item(), contrastive_loss.item() if use_image else None
        ...
        if not self.unified:
            for epoch in range(self.start_epoch, self.max_epochs):
                ...
                for batch in progress_bar:
                    loss, loss_ntp, contrastive_loss = self.train_step(batch, use_image=train_use_image)
                    epoch_loss += loss
                    ...
                    progress_bar.set_postfix(total_loss=loss, 
                ...
                epoch_contrastive_loss = epoch_contrastive_loss / max(contrastive_loss_batch_count, 1) if train_use_image else None
    ...
    def evaluate_loader(self, loader, use_image, prefix):
        ...
        avg_contrastive_loss = total_contrastive_loss / max(contrastive_loss_batch_count, 1) if use_image else None
